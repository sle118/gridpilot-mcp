...
- a widened but still structured MCP stdio host surface for session workbook discovery/connect/list/get/disconnect/create, inventory, workbook-name listing, name resolution/read/create/update/delete, query definition read, targeted refresh, probing, temp-query cleanup, query formula edit, table get/read/create/resize/append/replace/options, range read, range write, generic mutation permission grant/revoke/status tools, compatibility attached-session grant/revoke shims, plus structured host-side argument and invocation errors
...
- no broad attached-session workbook edit surface yet beyond query edits, name lifecycle, named-range reads, table get/read/create/resize/append/replace/options, range read/write, refresh, probe, and temp-query cleanup
- no broad unsafe-UI detection yet beyond the current readiness/interactivity-plus-edit/modal heuristics
- no broad workbook patching, formatting, worksheet lifecycle, or formula-aware worksheet surface yet
